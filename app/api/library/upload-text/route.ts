...
export async function POST(req: NextRequest) {
  ...
  const { title, text, tags: tagsRaw = '' } = await req.json();
  ...
  try {
    createDocument({ id, userId, title: title.trim(), tags, fileType: 'txt', filePath, fileSize: buffer.length });
  } catch (err) {
  ...
  updateDocumentTextCache(id, text.trim() + '[TTS_V3]');

  return NextResponse.json({ id, title: title.trim(), fileType: 'txt', tags, createdAt: new Date().toISOString() });
}
